    ...
    async with sem:
        for attempt in range(3):
            try:
                ...
                if attempt == 2:
                    print(f"Warning: Error fetching JPDB offset {offset} after 3 attempts: {type(e).__name__}: {e}")
                    return None
...
def scrape_jpdb(url: str) -> List[Dict[str, Any]]:
        ...
        try:
            with open(cache_path, "r", encoding="utf-8") as f:
                ...
                if isinstance(words, list) and words:
                    print(f" -> Found cached JPDB list ({len(words)} words). Using cached data.")
                    return words
        except Exception as e:
            print(f"Warning: Failed to read JPDB cache: {e}. Re-downloading...")

    print(f"Fetching JPDB vocabulary list from {base_url}...")
    ...
    if not first_html:
        print("Error: Failed to fetch first page from JPDB.")
        return []
    ...
    if not total_match:
        print("Warning: Could not parse total entry count from JPDB page.")
        total_entries = 50
    else:
    ...
    print(f" -> Expecting {total_entries} words across {total_pages} pages.")
    ...
    if any_failed:
        print(f"Error: Failed to download {failed_count}/{total_pages} pages. Will NOT cache.")
    else:
        if parsed_count >= total_entries - 5:
            print(f" -> Success: parsed {parsed_count}/{total_entries} words.")
            try:
                with open(cache_path, "w", encoding="utf-8") as f:
                    json.dump({"url": base_url, "title": title, "words": all_words}, f, ensure_ascii=False, indent=2)
                print(f" -> Cached to {cache_path}")
            except Exception as e:
                print(f"Warning: Failed to write JPDB cache: {e}")
        else:
            print(f"Warning: Word count mismatch (parsed {parsed_count}/{total_entries}). Will NOT cache.")
